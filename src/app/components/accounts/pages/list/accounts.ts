...
@Component({
  ...
  imports: [TableComponent, CreateModal, FormsModule, ReactiveFormsModule, NgIf, CreditAccount, DebitAccount, PriceFormatPipe, NgForOf],
...
export class AccountsComponent implements OnInit {
  ...
  header = [{ key: "index", label: "No" }, { key: "name", label: "Name" }, { key: "email", label: "Email" }, { key: "phone", label: "Phone" }, { key: "amount", label: "Balance" }, { key: "status", label: "Status" }];
  accounts: any[] = [];
  newAccount: any = {};
  ...
  save() {
    ...
    this.accounts.push({ ...this.accountForm.value, id: 'AC' + (this.accounts.length + 1).toString().padStart(4, '0'), status: 'Inactive' });
